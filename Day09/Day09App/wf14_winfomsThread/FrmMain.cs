...
namespace wf14_winfomsThread
{
    public partial class FrmMain : Form
    {
        ...
        {
            BackgroundWorker worker = sender as BackgroundWorker; // 인자값으로 BackgroundWorker 생성 ,얕은복사 - Worker랑 주소참소
        ...
        {
            // arg 0~91 91보다 크면 long에서 오버플로우 발생
            if (arg < 0 || arg > 91)
            {
                throw new ArgumentException("오류 0~91사이 입력");
            }
        ...
        // 백그라운드로 스레드 태스크 종료한 뒤 처리
        private void Worker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            PgbCalcuator.Value = e.ProgressPercentage; // 진행사항 표시
        }
        
        // 백그라운드 스레드 진행중 프로그래스바 표시
        ...
        {
            BtnStart.Enabled = false; // 시작버튼 종료시까지 누르지 못함
            BtnCancel.Enabled = true;

            number =  int.Parse(TxtNumber.Text);

            percent = 0;
            PgbCalcuator.Value = percent;
